...
test("should edit hotel", async ({ page }) => {
  ...
  await expect(page.locator('[name="name"]')).toHaveValue("Indian Getaways");
  await page.locator('[name="name"]').fill("Indian Getaways UPDATED");
  await page.getByRole("button", { name: "Save" }).click();
  await expect(page.getByText("Hotel Saved")).toBeVisible();

  await page.reload();
  await expect(page.locator('[name="name"]')).toHaveValue(
    "Indian Getaways UPDATED"
  );

  await page.locator('[name="name"]').fill("Indian Getaways");
  await page.getByRole("button", { name: "Save" }).click();
});
